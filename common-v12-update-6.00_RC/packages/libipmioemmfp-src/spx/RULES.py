...
#-------------------------------------------------------------------------------------------------------
# Rules to create libipmioemmfp package
def build_package_libipmioemmfp():
	...
	if retval != 0:
		return retval
	retval = Py_CopyFile(BUILD+"/"+PACKAGE+"/data/libipmioemmfp.so."+PrjVars["PKG_MAJOR"]+"."+PrjVars["PKG_MINOR"]+"."+PrjVars["PKG_AUX"],TEMPDIR+"/"+PACKAGE+"/tmp")
	if retval != 0:
		return retval
